...
# Now, your program should be able to accept alternative lists of options, 
# like Rock, Paper, Scissors, Lizard, Spock, or even a mlore complex list.
...
# Whatever list of options the user chooses, your program, obviously, should be able to identify 
# which option beats which, that is, the relationships between different options. 
...
# Now you have another list of options that doesn't include the "chosen" option 
# and has the different order of elements in it (first go the options following the chosen one in the original list, 
# after them are the ones that precede it). 
# So, in this "new" list, the first half of the options will be defeating the "chosen" option, 
# and the second half will get beaten by it.
...
# By looking at the list spock,rock,paper,scissors you realize that spock and rock will be beating the lizard, 
# and paper and scissors will get defeated by it. 
# For spock it'll be almost the same, but it'll get beaten by rock and paper, 
# and prevail over scissors and lizard. 
...
#   if yes, use the score specified in the 'rating.txt' for this user as a starting point 
#   for calculating the score in the current game. If no, start counting user's score from 0.
...
# - for each draw, add 50 points to the score. For each user's win, add 100 to his/her score. 
...
import sys
from classes.rockpaperscissors import RockPaperScissors
